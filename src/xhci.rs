...
// ============================================================
// PORTSC Registers (xHCI spec 5.4.8)
// ============================================================
...
impl GenericTrbEntry {
    ...
    fn set_trb_type(&mut self, trb_type: TrbType) {
        ...
        let ctrl = (ctrl & !(0x3F << 10)) | ((trb_type as u32) << 10);
    ...
    fn set_cycle_state(&mut self, cycle: bool) {
        let ctrl = self.control.read();
        let ctrl = if cycle { ctrl | 1 } else { ctrl & !1 };
        self.control.write(ctrl);
    }
    fn data(&self) -> u64 {
        self.data.read()
    ...
    fn cmd_address_device(input_context: &InputContext, slot: u8) -> Self {
        let mut trb = Self::default();
        trb.set_trb_type(TrbType::AddressDeviceCommand);
        trb.data.write(input_context as *const InputContext as u64);
...
impl TrbRing {
    ...
    fn current(&self) -> GenericTrbEntry {
        self.trb[self.current_index].read()
    }
    fn current_ptr(&self) -> usize {
        &self.trb[self.current_index] as *const Volatile<GenericTrbEntry> as usize
    }
...
impl InputContext {
    ...
    pub fn set_port_speed(&mut self, psi: UsbMode) -> Result<()> {
        self.device_ctx.set_port_speed(psi)
    }
    pub fn set_root_hub_port_number(&mut self, port: usize) -> Result<()> {
        self.device_ctx.set_root_hub_port_number(port)
    }
    pub fn set_last_valid_dci(&mut self, dci: usize) -> Result<()> {
        self.device_ctx.set_last_valid_dci(dci)
    }
}
...
impl EventRing {
    fn new() -> Result<Self> {
        let ring: IoBox<TrbRing> = IoBox::new();
        let erst_entry = EventRingSegmentTableEntry::new(&ring)?;
        let mut erst: IoBox<EventRingSegmentTableEntry> = IoBox::new();
        unsafe {
            *erst.get_unchecked_mut() = erst_entry;
        }
        Ok(Self {
            ring,
            erst,
...
impl Controller {
    fn new(mut regs: XhcRegisters) -> Result<Self> {
        ...
        // 6. Initialize primary event ring
        xhc.init_primary_event_ring()?;

        // 7. Initialize slots and contexts
    ...
    fn init_primary_event_ring(&mut self) -> Result<()> {
        unsafe { self.regs.rt_regs.get_unchecked_mut() }
            .init_irs(0, &mut self.primary_event_ring.lock())
    }
...
impl PciXhciDriver {
    ...
    async fn request_config_descriptor_and_rest(
        ...
        // First, get just the ConfigDescriptor to know total_length
        let config_buf = vec![0u8; size_of::<ConfigDescriptor>()];
        let mut config_buf = Box::into_pin(config_buf.into_boxed_slice());
        xhc.request_descriptor(
            slot,
            ctrl_ep_ring,
        ...
        let config_descriptor: ConfigDescriptor =
            ConfigDescriptor::copy_from_slice(&config_buf)?;
        // Now get the full descriptor with all Interface/Endpoint descriptors
        ...
        .await?;
        // Parse the buffer using DescriptorIterator
