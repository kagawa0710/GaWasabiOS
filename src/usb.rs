...
pub async fn request_device_descriptor(
    ...
    xhc.request_descriptor(slot, ctrl_ep_ring, UsbDescriptorType::Device, 0, 0, &mut buf)
        .await?;
...
pub async fn request_string_descriptor_zero(
    ...
    let buf = vec![0u8; 8];
    let mut buf = Box::into_pin(buf.into_boxed_slice());
    xhc.request_descriptor(slot, ctrl_ep_ring, UsbDescriptorType::String, 0, 0, &mut buf)
        .await?;
...
pub async fn request_config_descriptor_and_rest(
    ...
    let mut buf = Box::into_pin(buf.into_boxed_slice());
    xhc.request_descriptor(slot, ctrl_ep_ring, UsbDescriptorType::Config, 0, 0, &mut buf)
        .await?;
