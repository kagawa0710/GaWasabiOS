...
fn parse_hid_report_descriptor(report: &[u8]) -> Result<Vec<UsbHidReportInputItem>> {
    ...
    while let Some(prefix) = it.next() {
        ...
        match (&b_type, &b_tag) {
            ...
            (UsbHidReportItemType::Main, 0b1000) => {
                ...
                if let Some(usage_page) = usage_page {
                    let is_constant = (data_value & 0b1) == 1;
                    let is_array = (data_value >> 1) & 0b1 == 1;
                    let is_absolute = (data_value >> 2) & 0b1 == 0;
        ...
        // After Main item, clear local state
...
pub async fn start_usb_tablet(
    ...
    let (_config_desc, interface_desc, other_desc_list) =
        pick_interface_with_triple(descriptors, (3, 0, 0))
            .ok_or("No USB Tablet interface found")?;
    info!("USB tablet found");

    // Find HidDescriptor from the descriptor list
    ...
    // Request Report Descriptor with correct size
    ...
    // Calculate report size from parsed items
